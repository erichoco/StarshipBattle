...
function hurt(e) {
    ...
    if (!ship.e && !l.paused) {
        ...
        // var text = 'I survived ' + ((l.level || 1) - 1) + ' waves and got ' + l.p + ' points in Starship 13K:';
        // // Always use my domain as canonical link
        // var url = /^\w+:\/\/maettig\.com/.test(location.href)
        // 	? location.href
        // 	: 'http://maettig.com/code/canvas/starship-sorades-13 k/';
...
enemies.TYPES = [
    // Enemy number 0 is the smallest
    {
        render: function(c, a) {
            ...
            a.beginPath();
            for (var i = 5; i--;) {
                var x1 = c.width * (6 - i) / 11,
                    y1 = c.height * (6 - i) / 20;
                var x2 = c.width * (11 - i) / 26,
                    y2 = c.height * (1 + i) / 9;
    ...
    // Enemy number 1 is slightly bigger than enemy number 0, but almost the same
    {
        render: function(c, a) {
            ...
            a.shadowColor = a.strokeStyle;
            for (var i = 5; i--;) {
                var x1 = c.width * (5 - i) / 14 + a.shadowBlur,
                    y1 = c.height * (16 - i) / 17;
                var x2 = c.width * (8 - i) / 22,
                    y2 = c.height * (1 + i) / 11;
    ...
    // Enemy number 2 is probably the most dangerous, fires in all directions but does not aim
    {
        render: function(c, a) {
            ...
            for (var i = 0; i < Math.PI * 2; i += Math.PI / 4) {
                ...
                var x = c.width / 2 + Math.sin(i + d) * r,
                    y = c.width / 2 + Math.cos(i + d) * r;
                if (!i)
                    a.moveTo(x, y);
                else a.lineTo(x, y);
                d -= Math.PI / 1.45;
            ...
            for (var i = 0; i < Math.PI * 2; i += Math.PI / 4) {
                var r = c.width * .4;
                var x = c.width / 2 + Math.sin(i) * r,
                    y = c.width / 2 + Math.cos(i) * r;
                if (!i)
                    a.moveTo(x, y);
                else a.lineTo(x, y);
            }
...
a.fillRect(0, 0, c.width, c.height);
spawnText('LOADING', -1);
spawnText('STARSHIP Battle', 6 * 30);
...
function gameloop() {
    ...
    if (--ship.reload <= 0 && keys[88]) {
        ...
        // fire(0, -16);
        fire(16 * xAngle, -16 * yAngle);
        if (ship.weapon > 1) {
            var xAngle1 = Math.sin((ship.angle / 22 + 45 / 180) * Math.PI);
            var yAngle1 = Math.cos((ship.angle / 22 + 45 / 180) * Math.PI);
            var xAngle2 = Math.sin((ship.angle / 22 - 45 / 180) * Math.PI);
            var yAngle2 = Math.cos((ship.angle / 22 - 45 / 180) * Math.PI);
            fire(8 * 1.412 * xAngle1, -8 * 1.412 * yAngle1),
                fire(8 * 1.412 * xAngle2, -8 * 1.412 * yAngle2);
            // fire(8, -8);
            if (ship.weapon > 2) {
                fire(-16 * xAngle, 16 * yAngle);
                if (ship.weapon > 3)
                    fire(-16 * yAngle, -16 * xAngle),
                    fire(16 * yAngle, 16 * xAngle);
            }
        }
    ...
    if (l.bomb) {
        a.fillStyle = 'rgba(255,255,255,' + l.bomb--/ l.MAX_BOMB /
        2 + ')';
